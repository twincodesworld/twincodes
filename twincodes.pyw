# File: twincodes
# Date: 9/8/2017
# Author: ahmad hemmati (a.hahmad90@gmail.com)
# Purpose:
""" Twincodes is the name of a new encryption algorithm that can completely encrypt any type of file.
I am not a professional coder, and the following codes is only related to the test code for the Twincodes algorithm.
Non-standard naming and non-commenting, makes it hard to read codes quickly and I apologize for this.
But note that these codes are for the sole purpose of displaying and proving the function of the Twincodes algorithm and
is not a standard code for citation,so each person can create standardized and optimized codes based on the algorithm published
on the www.twincodesworld.com (For a better understanding of the current code, you should refer to the published algorithm.) """
...
def bit_bit(wsp):
    import binascii
    with open(wsp, 'rb') as my_file:
        my_bytes = my_file.read()
        convert_bytes = bin(int(binascii.hexlify(my_bytes), 16))[2:].zfill(8)
...
def b_number(gsp):
    ...
    return test_1_3_1
##################################
def tabdil(fsp,dsp,ssp,jav,lov):
    j=0
    jav
    for i in fsp:
        z=dsp.pop(i)
...
def mai_list(asp):
    ...
    while i<le:
        zoo=asp[i]
        joo=compare(zoo)
        woo= baisecod.index(joo)
        baisecod[woo].append(i)
        i+=1
##################################
def nahayi(zsp,xsp,nsp):
    msp=nsp+1
    j=0
    mov=[]
    while j<len(zsp):
...
def ejra(hgf):
    ...
    dir= hgf
    masir=os.path.dirname(dir)
    format_file=os.path.basename(dir)
    esme_file=os.path.splitext(dir)
    ttt=esme_file[0]
    esme=os.path.basename(ttt)
    bitbit=bit_bit(dir)
    intercode=inter_code(bitbit)
    if len(intercode)<=1049:
        print('this file is less than of that able to process')
        return
    tedad=0
    cods_akhar=[]
    test_2_2=[]
    list_3=[]
    while len(intercode)>=1050:
        list_1=borsh(intercode,cods_akhar)
        test_1=palayesh(intercode)
        if len(test_1)<= 1023:
            print('this file is less than of that able to process')
            return
        test_1_1=test_1[-1]
        test_1_3=number(test_1_1)
        hazf(test_1_3,test_1)
        test_2=test_1[-1]+1
        test_2_1=intercode[:test_2]
        test_2_3=intercode[test_2:]
        test_1_4=b_number(test_1_3)
        tabdil(test_1_4,test_2_1,test_1_3,test_2_2,list_3)
        test_2_2=[]
        if len(test_2_1) <= 1023:
            del cods_akhar[-1]
            del list_3[-1]
            break
        mai_list(test_2_1)
        list_2=nahayi(test_2_3,baisecod,tedad)
        with open("E:\\twincode\\" + esme + "_list2.txt", "wb") as fp:
            pickle.dump(list_2, fp)
        bitbi=bit_bit("E:\\twincode\\" + esme + "_list2.txt")
        intercode=inter_code(bitbi)
        tedad += 1
    reshte= str(list_2).strip('[]')
    reshte_kham=str(reshte).replace(',','')
    kham=reshte_kham.split()
    v_f_nahayi=elhaq(kham)
    with open(masir + "/" + esme +".txt", "wb") as fp:
        pickle.dump(v_f_nahayi, fp)
    with open("E:\\twincode\\" + esme + "_list1.txt", "wb") as fp:
        pickle.dump(cods_akhar, fp)
    with open("E:\\twincode\\" + esme + "_list3.txt", "wb") as fp:
        pickle.dump(list_3, fp)
    with open("E:\\twincode\\" + esme + "_mainlist.txt", "wb") as fp:
        pickle.dump(baisecod, fp)
    with open("E:\\twincode\\" + esme + "_formatfile.txt", "wb") as fp:
        pickle.dump(format_file, fp)
##################################
def daryaft():
    from tkinter import filedialog
    tempdir = filedialog.askopenfilename(parent=twin,  title='Choose a file')
    ejra(tempdir) 
from tkinter import *
twin = Tk()
twin.geometry("683x384")
twin.title("                                                                                                 twincodes")
"""twin.iconbitmap(default="f:\\T_Logo.ico")"""
labelframe = LabelFrame(twin, bg='white' , text=" twincodes >>>> encoding " , font= "Arial 8 bold")
labelframe.pack(fill="both",expand="yes")
left = Label(labelframe, bg='white' , height= 4 , text="Welcome to TWINCODES world!" , font= "Arial 24 bold")
left.pack()
left = Label(labelframe, bg='white' , height= 2 , text="Please click start button to coding your file!" , font= "Arial 8 bold")
left.pack()
dokme=Button(labelframe, text="start" , width=25 , command=daryaft)
dokme.pack()
left = Label(labelframe, bg='white' , height= 8 , text="Thank You!\n Think different to be different" , font= "Arial 8 bold")
left.pack()
twin.mainloop()
##################################
##################################
